...
.PHONY: help install up down stack-up stack-down migrate revision \
        worker mcp demo-pdf seed-demo seed-demo-books seed-minimal-lrrh lint fmt test \
...
help:
	...
	@echo "  seed-demo    load the Frog-King demo via the real flow (SEED_ARGS=...)"
	@echo "  seed-demo-books  seed both bundled demo books (Frog-King + Little Red Riding Hood)"
...
seed-minimal-lrrh:
	cd backend && .venv/bin/python scripts/seed_minimal_book.py

seed-demo-books:
	cd backend && .venv/bin/python scripts/seed_demo.py --pdf ../assets/books/the_frog_king.pdf --title "The Frog-King"
	cd backend && .venv/bin/python scripts/seed_demo.py --pdf ../assets/books/little_red_riding_hood.pdf --title "Little Red Riding Hood" --art-direction "woodland storybook"
