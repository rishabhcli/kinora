...
A third-party video-provider plugin declares its own ``version`` and the range
of Kinora plugin-API versions it is compatible with (``kinora_api``). The SDK
therefore needs deterministic version comparison and constraint satisfaction
without pulling in a third-party semver package — and, per the final-round
rules, without importing the parallel ``app.platform.plugins`` implementation
(that subsystem may not be merged). This is an intentionally self-contained
copy covering exactly the subset the SDK uses:
...
* :class:`Version` — ``MAJOR.MINOR.PATCH`` with an optional ``-prerelease``,
  ...
  numerically; a prerelease is *lower* than its release; build metadata is
  ignored for precedence).
* :class:`VersionRange` — a comma-separated conjunction of comparator terms
  (``>=1.2.0``, ``<2.0.0``, ``==1.4.1``, ``!=1.3.0``, ``^1.2``, ``~1.2.3``,
  ``1.2.x``, ``*``).

Everything is pure and total: a malformed string raises
:class:`~app.video.plugins.errors.ManifestError`, never silently coerces.
"""
...
class Version:
    ...
    def parse(cls, text: str) -> Version:
        ...
        if not isinstance(text, str):
            raise ManifestError(f"version must be a string: {text!r}")
        m = _CORE_RE.match(text.strip())
        if m is None:
            raise ManifestError(f"invalid semantic version: {text!r}")
...
class VersionRange:
    ...
    def parse(cls, text: str) -> VersionRange:
        ...
        if not isinstance(text, str):
            raise ManifestError(f"version range must be a string: {text!r}")
        spec = text.strip()
        if spec in ("", "*", "x", "X"):
        ...
        for part in spec.split(","):
            term = part.strip()
            if not term:
                raise ManifestError(f"empty term in version range: {text!r}")
...
def _parse_term(term: str) -> tuple[_Comparator, ...]:
    ...
    if m is None:  # pragma: no cover - regex always matches non-empty
        raise ManifestError(f"invalid range term: {term!r}")
...
def _caret(rest: str) -> tuple[_Comparator, ...]:
    """``^1.2.3`` → ``>=1.2.3, <2.0.0`` (``^0.x`` pins minor; ``^0.0.z`` patch)."""
