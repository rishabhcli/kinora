...
reset_demo() {
  # DB reset only (no build/up) — fast enough for the console's 60s timeout.
  # The render-worker is stopped first: while it is actively rendering it holds
  # row locks on shots/render_jobs, so the reset would otherwise block on lock
  # contention until the command times out. lock_timeout makes any residual
  # contention fail fast and visibly instead of hanging.
  {
    ...
    $C exec -T postgres psql -v ON_ERROR_STOP=1 -U kinora -d kinora -c \
      "SET lock_timeout='8s'; BEGIN; \
       ...
       WHERE book_id='$BID'; \
       COMMIT;"
    echo "--- start render-worker ---"; $C start render-worker
    sleep 4
