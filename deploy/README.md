...
**Alibaba OSS** (object storage) and **DashScope / Model Studio** (Wan 2.7 video
synthesis + the Qwen crew), designed to run on **ECS or Function Compute** as a
queue worker.
...
| **DashScope** (Model Studio) | `app.providers.video.VideoProvider` → Wan 2.7 async video-synthesis; the rest of the Qwen crew | `render_shot_to_oss()` + the pipeline inside `build_worker()` |
...
- **`render_shot_to_oss(spec)`** — the minimal §12.6 demonstration: render one
  shot with DashScope Wan and `put_object` the clip to OSS, returning an
...
pgvector, Tair/Redis, and ECS nodes for api + render-worker + mcp). Each ECS node
runs this image with its role command via cloud-init. See
...
Flip `KINORA_LIVE_VIDEO=1`, enqueue one shot, and record the worker rendering a
real Wan clip through DashScope and writing it to OSS — then link this file in
the submission (kinora.md §12.6 / §17).
