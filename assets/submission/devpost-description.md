...
- **Reliability:** idempotent jobs, bounded retries, provider polling with backoff, stream-ready video normalization, persistent media, health checks, and explicit reader-only degradation when video is unavailable.
...
The hardest challenge was keeping the demo honest while making it resilient. Provider URLs expire, generated MP4s are not always arranged for progressive playback, image quotas and video latency behave differently, long novels overwhelm naive prompts, and speculative rendering can create unnecessary work. Kinora responds with immediate downloads, H.264/AAC normalization with `faststart`, typed provider profiles, separate image/video handling, source-span retrieval, explicit generation gates, and cancelable scheduling.
